...
public class MarqueeView extends ViewFlipper {
    ...
    private View createView(int position) {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.marquee_item_layout, null);
        TextView oilName = view.findViewById(R.id.oil_name);
        TextView oilPrice = view.findViewById(R.id.oil_price);
        TextView oilPercent = view.findViewById(R.id.oil_percent);
        ImageView gvUpDown = view.findViewById(R.id.gv_up_down);

        String oilPriceAfter = marquee.getOilPrice();
        String oilNormalPriceBefor = marquee.getOilNormal();

//        > < =  三种情况

        if ((!TextUtils.isEmpty(oilPriceAfter) && (!TextUtils.isEmpty(oilNormalPriceBefor)))) {

            Double afterPrice = Double.parseDouble(oilPriceAfter);

            Double beforePrice = Double.parseDouble(oilNormalPriceBefor);

            int retval = beforePrice.compareTo(afterPrice);

            if (retval > 0) {

                gvUpDown.setBackgroundResource(R.mipmap.arrow_down_blue);
            } else if (retval < 0) {
                gvUpDown.setBackgroundResource(R.mipmap.arrow_up_blue);
            } else {
                gvUpDown.setBackgroundResource(R.mipmap.arrow_eq_blue);
            }
        } else {
            gvUpDown.setVisibility(View.GONE);

        }


        oilName.setText(marquee.getOilTitle());
        oilPrice.setText(marquee.getOilPrice()+ "元/L");
        oilPercent.setText(marquee.getOilPercent()+ "元");
